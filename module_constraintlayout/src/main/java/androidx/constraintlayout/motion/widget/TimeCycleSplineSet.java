...
public abstract class TimeCycleSplineSet {
    ...
    public float get(float pos, long time, View view, KeyCache cache) {
        ...
        this.last_cycle = (float) ((((double) this.last_cycle) + ((((double) (time - this.last_time)) * 1.0E-9d) * ((double) period))) % 1.0d);
        cache.setFloatValue(view, this.mType, 0, this.last_cycle);
        this.last_time = time;
        float v = this.mCache[0];
        float calcWave = (v * calcWave(this.last_cycle)) + this.mCache[2];
    ...
    public float calcWave(float period) {
        switch (this.mWaveShape) {
            ...
            case 5:
                return (float) Math.cos((double) (VAL_2PI * period));
            case 6:
                float x = 1.0f - Math.abs(((period * 4.0f) % 4.0f) - 2.0f);
                return 1.0f - (x * x);
            default:
                return (float) Math.sin((double) (VAL_2PI * period));
        }
    }
